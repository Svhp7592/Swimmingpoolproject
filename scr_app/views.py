from django.shortcuts import render,HttpResponse  # Import functions for rendering templates and sending responses
from scr_app.models import *  # Import all models from your app
from django.conf import settings # Import settings to access project configuration
from django.core.files.storage import FileSystemStorage # For file upload handling
...
def home(request): # Home page view
    ...
    return render(request,'home.html') # Render home.html template  # Admin home page view
...
def logins(request):
    if request.method=='POST': # Check if the form is submitted
        uname=request.POST['uname'] # Get username
        psw=request.POST['psw'] # Get password
...
def manage_parking_location(request):
    q=parking_location.objects.all() # Fetch all the slots from database
...
def user_booking(request,id,am):
    ...
    if request.method=='POST':
        ...
        t=booking(starting_date=sd,starting_time=st,ending_date='pending',ending_time='pending',user_id=request.session['user_id'],slot_id=id,status='occupied',amount=am)
        t.save()
        
        f=slot.objects.get(slot_id=id)
        if f:
            f.slot_status='occupied'
            f.save()
